...
## What This Is
Network diagnostic toolkit for a UniFi Dream Router (UDR) running UniFi OS 7+.
...
- Connect: `ssh udr`.
- UDR has MongoDB on port 27117 (no auth, localhost-only) with all UniFi data.

## Project Layout
```
scripts/
  collect-tcpdump.sh      — SSH to UDR, capture with BPF filter, pull pcap back
  collect-logs.sh         — SSH to UDR, grab filtered logs
  collect-clients.sh      — Pull client list from UDR (ARP + DHCP leases)
  ...
  analyze-conntrack.sh    — SSH to UDR, dump filtered conntrack table
...
docs/
  ...
  unifi_config.example.md — example of an annotated UDR config digest
  common-issues.md        — known issue patterns and diagnostic steps
```

## MongoDB Data Source (collect-unifi-db.sh)
UDR runs MongoDB 3.6 on port 27117. Two key databases:
...
`./scripts/collect-unifi-db.sh clients-lean` to bootstrap from the live UDR.
